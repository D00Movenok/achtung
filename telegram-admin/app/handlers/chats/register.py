...
from .keyboards import chats_callback, types_callback, edit_callback
...
def register_chats(dp: aiogram.Dispatcher):
    ...
    dp.register_callback_query_handler(del_chat, edit_callback.filter(
                                       action='delete'), chat_id=ADMIN_ID)
    dp.register_callback_query_handler(back_to_chat, edit_callback.filter(
