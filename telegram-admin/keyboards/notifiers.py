...
def notifiers_get_keyboard(page: int = 0):
    markup = InlineKeyboardMarkup(row_width=2)

    previous_callback_data = notifiers_callback.new(action='go_to',
                                                    page=page-1)
    page_callback_data = notifiers_callback.new(action='page', page=page)
    next_callback_data = notifiers_callback.new(action='go_to', page=page+1)
